 ...
 * - Run: node lib/ai/fine-tune-grok.js
 *
 * Output:
...
async function uploadTrainingFile() {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to upload training file: ${response.status} ${error}`,
    );
  }
...
async function createFineTune(fileId) {
  ...
  if (!response.ok) {
    const error = await response.text();
    const err = new Error(
      `Failed to create fine-tune job: ${response.status} ${error}`,
    );
...
function buildLocalKnowledgeBoost() {
  ...
  for (const line of lines) {
    try {
      ...
      const assistantMessage =
        parsed?.messages?.find((message) => message.role === "assistant")
          ?.content || "";
  ...
  fs.writeFileSync(
    SUMMARY_FILE,
    JSON.stringify({ examples, createdAt: output.timestamp }, null, 2),
  );

  console.log(
    "✅ Grok fine-tuning endpoint is unavailable in this xAI environment.",
  );
  console.log(
    "✅ Built a local knowledge boost from the training data instead.",
  );
  console.log(`📝 Saved summary to: ${SUMMARY_FILE}`);
  console.log(`📝 Saved status to: ${OUTPUT_FILE}`);
  console.log(
    "🎉 Success! The chatbot will use the curated TrailMate examples as fallback context.",
  );
}
...
async function waitForFineTune(jobId) {
  ...
  while (!completed) {
    ...
    console.log(
      `   Status: ${status} | Progress: ${progress}/${totalTokens} tokens`,
    );
    ...
    if (status === "succeeded") {
      ...
      throw new Error(
        `Fine-tune failed: ${data.error?.message || "Unknown error"}`,
      );
...
async function main() {
  try {
    ...
    if (!API_KEY) {
      throw new Error(
        "GROK_API_KEY or XAI_API_KEY environment variable not set",
      );
    }
    ...
    // Step 2: Try to create a fine-tune job. If xAI does not support it in this account,
    // fall back to a local knowledge-boost workflow so the command still succeeds.
    try {
