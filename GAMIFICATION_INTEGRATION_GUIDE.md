...
async function checkYourBadge(db, guide) {
  // Add custom logic
  const tripCount = await db.collection('bookings')
    .countDocuments({ 
      guideId: guide._id, 
      status: 'completed' 
    })
...
async function checkSeasonalBadge(db, guide, season) {
  const trips = await db.collection('bookings')
    .find({ guideId: guide._id })
    .toArray()

  const seasonTrips = trips.filter(t => 
    getSeasonFromDate(new Date(t.createdAt)) === season
  )
...
```typescript
const badgeStats = await db.collection('guide_badges')
  .aggregate([
...
```typescript
const topGuides = await db.collection('guide_badges')
  .aggregate([
    {
      $group: {
        _id: '$guideId',
...
```typescript
const velocity = await db.collection('badge_achievements')
  .aggregate([
    {
      $match: {
        guideId: guideId,
...
async function getCachedBadges(guideId) {
  ...
  const badges = await db.collection('guide_badges')
    .find({ guideId })
    .toArray()
