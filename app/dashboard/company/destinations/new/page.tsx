...
export default function NewDestinationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, publish = false) => {
    ...
    try {
      ...
      console.log("Submitting destination with payload:", { ...payload, userId: user._id, role: user.role })
      ...
      if (!res.ok) {
        throw new Error(data.error || `Failed to create destination (${res.status})`)
      }

      // Success! Redirect to dashboard
      alert(publish ? "Tour published successfully!" : "Tour saved as draft!")
  ...
  return (
    <div className="flex min-h-screen bg-secondary">
      <DashboardSidebar role="company" user={{ name: user ? `${user.firstName} ${user.lastName}` : "Company", email: user?.email || "" }} />

      <div className="flex-1 ml-64">
        <DashboardTopbar title="Add New Tour" />
